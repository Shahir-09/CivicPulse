...
import { useGnaniVoice } from '../hooks/useGnaniVoice';
import { isGnaniEnabled } from '../services/gnaniService';
...
export default function ReportPage() {
  ...
  // Gnani.ai Hook Integration
  const gnaniVoice = useGnaniVoice({
    onTranscriptionSuccess: (text) => {
      setVoiceTranscript(text);
      processCleanVoice(text);
    },
    onTranscriptionError: (err) => {
      logger.error("Gnani transcription failed, falling back...", err);
      toast.error("Gnani voice integration failed. Falling back to native recognition...");
      // Trigger native speech recognition fallback
  ...
  const startNativeVoiceRecording = () => {
  ...
  // Main voice recording trigger
  const startVoiceRecording = () => {
    if (isGnaniEnabled()) {
      if (gnaniVoice.isRecording) {
        gnaniVoice.stopRecording();
      } else {
        gnaniVoice.startRecording();
        toast('Recording with Gnani.ai voice assistant...', { icon: '🎤' });
      }
    } else {
      startNativeVoiceRecording();
    }
              ...
              <div style={{ display: 'flex', gap: '8px' }}>
                ...
                <button
                  ...
                  style={{
                    ...
                    gap: '4px',
                    minHeight: 'auto',
                    background: (voiceActive || gnaniVoice.isRecording) ? 'var(--danger-subtle)' : 'var(--surface-2)',
                    borderColor: (voiceActive || gnaniVoice.isRecording) ? 'var(--danger)' : 'var(--border)'
                  }}
                  onClick={startVoiceRecording}
                  disabled={voiceCleaning || gnaniVoice.isTranscribing}
                >
                  <Mic
                    size={12}
                    style={{
                      color: (voiceActive || gnaniVoice.isRecording) ? 'var(--danger)' : 'var(--text-1)',
                      animation: (voiceActive || gnaniVoice.isRecording) ? 'pulse 1.5s infinite' : 'none'
                    }}
                  />
                  {gnaniVoice.isRecording ? 'Stop Recording' : 
                   gnaniVoice.isTranscribing ? 'Transcribing...' :
                   voiceActive ? t('report.voice.listening') : 
                   (voiceCleaning ? t('report.voice.cleaning') : t('report.voice.speak'))}
                </button>
              </div>
            </label>
            <textarea 
